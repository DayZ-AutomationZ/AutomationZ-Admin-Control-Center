...
Originally built for DayZ, this tool works just as well for Arma, other modded games, VPS servers, server parks, and any workflow where folders need to stay in sync. It can send a Markerfile to perfectly work in sync with the Restart Companion. The File is being uploaded after a mod or mods are updated to the server, the file is in /dayzstandalone and the Restart Companion watches the file for change in timestamps, if a mod is updated the file is uploaded after a delay and the restart companion will detect a change and restarts your server, Local or rcon. 
